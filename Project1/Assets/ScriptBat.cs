...
public class ScriptBat : MonoBehaviour {
	...
	void Update () {
			...
			if (y > (text1_y - buffer) && y < (text1_y + buffer)) {
				if (z > (text1_z - buffer) && z < (text1_z + buffer)){
					...
					if (Input.GetKeyDown (KeyCode.N)) {
						Camera.main.gameObject.transform.position =new Vector3 (1240,171,223);
		...
		if (x > (text5_x - buffer) && x < (text5_x + buffer)) {
			if (y > (text5_y - buffer) && y < (text5_y + buffer)) {
				if (z > (text5_z - buffer) && z < (text5_z + buffer)){
